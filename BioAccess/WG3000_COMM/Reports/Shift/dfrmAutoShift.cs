namespace WG3000_COMM.Reports.Shift
{
    ...
    public partial class dfrmAutoShift : frmBioAccess
    {
        ...
        private void _dataTableLoad()
        {
            ...
            else
            {
                ...
                cmdText = " SELECT t_b_Group.f_GroupName,t_b_Consumer.f_ConsumerID, t_b_Consumer.f_ConsumerName, LTRIM(([f_ConsumerNo]) +'- '+ [f_ConsumerName]) as [f_UserFullName]  FROM [t_b_Consumer]  LEFT OUTER JOIN t_b_Group ON  t_b_Group.f_GroupID = t_b_Consumer.f_GroupID  WHERE f_AttendEnabled = 1 ";
                cmdText = cmdText + " AND f_ShiftEnabled > 0 ";
        ...
        private void _dataTableLoad_Acc()
        {
            ...
            string str = "";
            this.dsConsumers = new DataSet("Users");
            str = " SELECT t_b_Group.f_GroupName,t_b_Consumer.f_ConsumerID, t_b_Consumer.f_ConsumerName, LTRIM(([f_ConsumerNo]) +'- '+ [f_ConsumerName]) as [f_UserFullName]  FROM [t_b_Consumer]  LEFT OUTER JOIN t_b_Group ON  ( t_b_Group.f_GroupID = t_b_Consumer.f_GroupID ) WHERE f_AttendEnabled = 1 ";
            selectCommand = new OleDbCommand(str + " AND f_ShiftEnabled > 0 ", connection);
            ...
            try
            {
                if (wgAppConfig.IsAccessDB)
                {
                    selectCommand = new OleDbCommand("SELECT [f_ShiftID] & '-' & [f_ShiftName] as f_ShiftFullName, [f_ShiftID] from t_b_ShiftSet order by f_ShiftID ASC", connection);
                }
                else
                {
                    selectCommand = new OleDbCommand("SELECT CONVERT(nvarchar(50),[f_ShiftID]) + case when [f_ShiftName] IS NULL Then '' ELSE   '-' + [f_ShiftName] end  as f_ShiftFullName, [f_ShiftID] from t_b_ShiftSet order by f_ShiftID ASC", connection);
                }
                try
                {
            ...
            try
            {
                if (this.arrSelectedShiftID.Count > 0)
                {
                    ...
                    if (wgAppConfig.IsAccessDB)
                    {
                        ...
                        {
                            num = acc.shift_rule_checkValid(shiftRule.Length, shiftRule);
                            if (num == 0)
                            {
                    ...
                    {
                        num = shift.shift_rule_checkValid(shiftRule.Length, shiftRule);
                        if (num == 0)
                        {
                            this.ProgressBar1.Maximum = numArray2.Length;
                            for (num2 = 0; num2 <= (numArray2.Length - 1); num2++)
                            {
                                this.ProgressBar1.Value = num2;
                                num = shift.shift_arrangeByRule(numArray2[num2], dateStart, dateEnd, shiftRule.Length, shiftRule);
                                if (num != 0)
                                {
                                    XMessageBox.Show(this, shift.errDesc(num), wgTools.MSGTITLE, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                    break;
                                }
                            }
                            if (num == 0)
