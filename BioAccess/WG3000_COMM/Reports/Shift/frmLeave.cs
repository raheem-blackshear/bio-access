namespace WG3000_COMM.Reports.Shift
{
    ...
    public partial class frmLeave : frmBioAccess
    {
        ...
        {
            using (dfrmLeaveAdd add = new dfrmLeaveAdd())
            {
                add.ShowDialog();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (this.dgvMain.RowCount > 0)
            {
                ...
                {
                    index = this.dgvMain.SelectedRows[0].Index;
                    wgAppConfig.runUpdateSql(" DELETE FROM t_d_Leave WHERE [f_NO]= " + this.dgvMain[0, index].Value.ToString());
                }
                else
                {
                    foreach (DataGridViewRow row in this.dgvMain.SelectedRows)
                    {
                        wgAppConfig.runUpdateSql(" DELETE FROM t_d_Leave WHERE [f_NO]= " + row.Cells[0].Value.ToString());
                    }
                }
                this.btnQuery_Click(sender, null);
            }
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string strsql = wgAppConfig.getSqlFindNormal(((((" SELECT t_d_Leave.f_NO, t_b_Group.f_GroupName, " + "       t_b_Consumer.f_ConsumerNO AS f_ConsumerNO, ") + " t_b_Consumer.f_ConsumerName AS f_ConsumerName, " + " CONVERT(DateTime , [f_Value]) AS f_from, ") + " [f_Value1] AS f_from1, " + " CONVERT(DateTime , [f_Value2]) AS f_to, ") + " [f_Value3] AS f_to1, " + " t_d_Leave.[f_HolidayType], ") + " t_d_Leave.f_Notes, " + " t_b_Consumer.f_ConsumerID  ", "t_d_Leave", this.getSqlOfDateTime("t_d_Leave.f_DateYM"), groupMinNO, groupIDOfMinNO, groupMaxNO, findName, findCard, findConsumerID);
        ...
        private void btnQuery_Click_Acc(object sender, EventArgs e)
        {
            ...
            int findConsumerID = 0;
            this.userControlFind1.getSqlInfo(ref groupMinNO, ref groupIDOfMinNO, ref groupMaxNO, ref findName, ref findCard, ref findConsumerID);
            string strsql = wgAppConfig.getSqlFindNormal(((((" SELECT t_d_Leave.f_NO, t_b_Group.f_GroupName, " + "       t_b_Consumer.f_ConsumerNO AS f_ConsumerNO, ") + " t_b_Consumer.f_ConsumerName AS f_ConsumerName, " + " CDate([f_Value]) AS f_from, ") + " [f_Value1] AS f_from1, " + " CDate([f_Value2]) AS f_to, ") + " [f_Value3] AS f_to1, " + " t_d_Leave.[f_HolidayType], ") + " t_d_Leave.f_Notes, " + " t_b_Consumer.f_ConsumerID  ", "t_d_Leave", this.getSqlOfDateTime("t_d_Leave.f_DateYM"), groupMinNO, groupIDOfMinNO, groupMaxNO, findName, findCard, findConsumerID);
            this.reloadData(strsql);
        }
        ...
        {
            string str = "";
            str = "  ([f_Value2]>= " + wgTools.PrepareStr(this.dtpDateFrom.Value.ToString("yyyy-MM-dd")) + ")";
            if (str != "")
            {
                str = str + " AND ";
            }
            return (str + "  ( [f_Value] <= " + wgTools.PrepareStr(this.dtpDateTo.Value.ToString("yyyy-MM-dd  23:59:59")) + ")");
        }
        ...
        private DataTable loadDataRecords(int startIndex, int maxRecords, string strSql)
        {
            ...
            {
                strSql = strSql + string.Format(" AND t_d_Leave.f_NO > {0:d}", this.recIdMax);
            }
            else
            {
                strSql = strSql + string.Format(" WHERE t_d_Leave.f_NO > {0:d}", this.recIdMax);
            }
            strSql = strSql + " ORDER BY f_NO ";
