namespace WG3000_COMM.Reports.Shift
{
    ...
    public partial class dfrmLeaveAdd : frmBioAccess
    {
            ...
            using (SqlConnection connection = new SqlConnection(wgAppConfig.dbConString))
            {
                using (SqlCommand command = new SqlCommand(cmdText, connection))
                {
                    ...
                    while (count < this.dgvSelectedUsers.Rows.Count)
                    {
                        ...
                        cmdText = "INSERT INTO [t_d_Leave] (f_ConsumerID, f_Value ,f_Value1,f_Value2,f_Value3,f_HolidayType,f_Notes)";
                        cmdText = (((((((cmdText + " SELECT t_b_Consumer.f_ConsumerID ") + ", " + wgTools.PrepareStr(this.dtpStartDate.Value.ToString("yyyy-MM-dd")) + " AS [f_Value] ") + ", " + wgTools.PrepareStr(this.cboStart.Text) + " AS [f_Value1] ") + ", " + wgTools.PrepareStr(this.dtpEndDate.Value.ToString("yyyy-MM-dd")) + " AS [f_Value2] ") + ", " + wgTools.PrepareStr(this.cboEnd.Text) + " AS [f_Value3] ") + ", " + wgTools.PrepareStr(this.cboHolidayType.Text) + " AS [f_HolidayType] ") + ", " + wgTools.PrepareStr(this.txtf_Notes.Text) + " AS [f_Notes] ") + " FROM t_b_Consumer";
                        if (str != "")
                        {
        ...
        Label_029A:
            if (flag)
            {
                XMessageBox.Show(string.Concat(new object[] { (sender as Button).Text, "  ", num, "  ", CommonStr.strUnit, "  ", CommonStr.strSuccessfully }), wgTools.MSGTITLE, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
            else
            {
            ...
            if (wgAppConfig.IsAccessDB)
            {
                using (OleDbConnection connection = new OleDbConnection(wgAppConfig.dbConString))
                {
                    using (OleDbCommand command = new OleDbCommand(cmdText, connection))
                    {
                        ...
                        {
                            adapter.Fill(this.ds, "t_a_HolidayType");
                        }
                    }
                }
            ...
            else
            {
                using (SqlConnection connection2 = new SqlConnection(wgAppConfig.dbConString))
                {
                    using (SqlCommand command2 = new SqlCommand(cmdText, connection2))
                    {
                        ...
                        {
                            adapter2.Fill(this.ds, "t_a_HolidayType");
                        }
                    }
                }
