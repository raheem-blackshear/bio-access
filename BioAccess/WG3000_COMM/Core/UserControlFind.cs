namespace WG3000_COMM.Core
{
    ...
    partial class UserControlFind : UserControl
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (wgAppConfig.IsAccessDB)
                str = " SELECT f_ConsumerID, f_ConsumerNO, f_ConsumerName, f_CardNO, 0 as f_Selected, f_GroupID, '(' + LTRIM(f_ConsumerNO) + ')-' +  f_ConsumerName " + /* '-' + IIF(ISNULL( f_CardNO),'-',CSTR(f_CardNO)) */ "As f_ConsumerFull FROM t_b_Consumer ";
            else
                str = " SELECT f_ConsumerID, f_ConsumerNO, f_ConsumerName, f_CardNO, 0 as f_Selected, f_GroupID, '(' + LTRIM(f_ConsumerNO) + ')-' +  f_ConsumerName " + /* '-' + CASE WHEN f_CardNO IS NULL THEN '-' ELSE CONVERT(nvarchar(50),f_CardNO) END */ " As f_ConsumerFull FROM t_b_Consumer ";
