namespace WG3000_COMM.Basic
{
    ...
    public partial class dfrmOperatorDepartmentsConfiguration : frmBioAccess
    {
        ...
        private void _dataTableLoad()
        {
            ...
            else
            {
                ...
                {
                    using (SqlCommand command = new SqlCommand("Select * from t_b_Group where f_GroupID IN (SELECT f_GroupID FROM t_b_Group4Operator Where f_OperatorId = " + this.operatorId.ToString() + " ) ", connection))
                    {
                        using (SqlCommand command2 = new SqlCommand("Select * from t_b_Group where f_GroupID NOT IN (SELECT f_GroupID FROM  t_b_Group4Operator Where f_OperatorId = " + this.operatorId.ToString() + " ) ", connection))
                        {
        ...
        private void _dataTableLoad_Acc()
        {
            ...
            {
                using (OleDbCommand command = new OleDbCommand("Select * from t_b_Group where f_GroupID IN (SELECT f_GroupID FROM t_b_Group4Operator Where f_OperatorId = " + this.operatorId.ToString() + " ) ", connection))
                {
                    using (OleDbCommand command2 = new OleDbCommand("Select * from t_b_Group where f_GroupID NOT IN (SELECT f_GroupID FROM  t_b_Group4Operator Where f_OperatorId = " + this.operatorId.ToString() + " ) ", connection))
                    {
                        this.ds = new DataSet("Users-Doors");
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dtSelectedGroups.Rows.Count > 0)
                {
                    for (int i = 0; i <= (dtSelectedGroups.Rows.Count - 1); i++)
                    {
                        wgAppConfig.runUpdateSql((((" INSERT INTO t_b_Group4Operator" + " (f_GroupID, f_OperatorID) ") + " Values(" + dtSelectedGroups.Rows[i]["f_GroupID"]) + " ," + this.operatorId) + " )");
                    }
                }
                base.Close();
            }
