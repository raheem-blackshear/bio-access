namespace WG3000_COMM.ExtendFunc.Patrol
{
    ...
    public partial class frmPatrolTaskData : frmBioAccess
    {
        ...
        {
            using (dfrmPatrolTaskAutoPlan plan = new dfrmPatrolTaskAutoPlan())
            {
                plan.ShowDialog();
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            if (XMessageBox.Show(string.Format(CommonStr.strAreYouSure + " {0} ?", str), wgTools.MSGTITLE, MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                {
                    num = patrol.shift_arrange_delete(0, DateTime.Parse("2000-1-1"), DateTime.Parse("2050-12-31"));
                    if (num == 0)
                    {
                        XMessageBox.Show(this, "OK!", wgTools.MSGTITLE, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                        this.btnQuery_Click(sender, null);
        ...
        {
            using (dfrmPatrolTaskDelete delete = new dfrmPatrolTaskDelete())
            {
                                ...
                                if (edit.ShowDialog() == DialogResult.OK)
                                {
                                    ...
                                    if (wgAppConfig.IsAccessDB)
                                    {
                                        using (comPatrol_Acc acc = new comPatrol_Acc())
                                        {
                                            ...
                                            long num2 = acc.shift_arrange_update(Convert.ToInt32(row.Cells["f_ConsumerID"].Value), dateShift, routeID);
                                            if (num2 == 0L)
                                            {
                                                if (routeID == 0)
                                                {
                                    ...
                                    using (comPatrol patrol = new comPatrol())
                                    {
                                        ...
                                        long num3 = patrol.shift_arrange_update(Convert.ToInt32(row.Cells["f_ConsumerID"].Value), time2, routeID);
                                        if (num3 == 0L)
                                        {
                                            if (routeID == 0)
                                            {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            if (wgAppConfig.IsAccessDB)
            {
                str3 = " SELECT t_d_PatrolPlanData.f_RecID, t_b_Group.f_GroupName, ";
                str3 = (str3 + "       t_b_Consumer.f_ConsumerNO AS f_ConsumerNO, ") + " t_b_Consumer.f_ConsumerName AS f_ConsumerName, " + " t_d_PatrolPlanData.f_DateYM, ";
                for (int i = 1; i < 0x1f; i++)
                {
                    str3 = str3 + string.Format(" CSTR(t_d_PatrolPlanData.f_RouteID_{0:d2}) as f_RouteID_{0:d2}, ", i);
                }
                str3 = str3 + "CSTR(t_d_PatrolPlanData.f_RouteID_31) as f_RouteID_31 " + " ,t_b_Consumer.f_ConsumerID  ";
            }
            else
            {
                str3 = " SELECT t_d_PatrolPlanData.f_RecID, t_b_Group.f_GroupName, ";
                ...
                {
                    str3 = str3 + string.Format(" CONVERT(nvarchar(3),  t_d_PatrolPlanData.f_RouteID_{0:d2}) as f_RouteID_{0:d2}, ", j);
                }
                str3 = str3 + "CONVERT(nvarchar(3),  t_d_PatrolPlanData.f_RouteID_31) as f_RouteID_31 " + " ,t_b_Consumer.f_ConsumerID  ";
            }
            string strsql = wgAppConfig.getSqlFindNormal(str3, "t_d_PatrolPlanData", this.getSqlOfDateTime("t_d_PatrolPlanData.f_DateYM"), groupMinNO, groupIDOfMinNO, groupMaxNO, findName, findCard, findConsumerID);
        ...
        private DataTable loadDataRecords(int startIndex, int maxRecords, string strSql)
        {
            ...
            {
                strSql = strSql + string.Format(" AND t_d_PatrolPlanData.f_RecID > {0:d}", this.recIdMax);
            }
            else
            {
                strSql = strSql + string.Format(" WHERE t_d_PatrolPlanData.f_RecID > {0:d}", this.recIdMax);
            }
            strSql = strSql + " ORDER BY t_d_PatrolPlanData.f_RecID ";
