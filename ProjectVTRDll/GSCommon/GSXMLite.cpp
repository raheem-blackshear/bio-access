...
/* xml file을 load 한다.                                                */
/************************************************************************/
BOOL _tagXMLNode::LoadFile(LPSTR szFileName)
{
	FILE* pFile = fopen(szFileName, "r, ccs=UTF-8");
	if (pFile == NULL)
	{
		CString str;
		str.Format(L"Failed to load %S XML file", szFileName);
		MessageBox(NULL, str, L"Error", MB_OK | MB_ICONERROR);
		return FALSE;
	}

	fseek(pFile, 0, SEEK_END);
	long file_size = ftell(pFile) * 2;
	fseek(pFile, 0, SEEK_SET);

	WCHAR* buffer = new WCHAR[file_size + 1];
	memset(buffer, '\0', file_size);
	fread(buffer, sizeof(char), file_size, pFile);
	fclose(pFile);

	if (Load(buffer) == NULL)
	{
		MessageBox(NULL, _T("Config XML file type error."), _T("Error"), MB_OK | MB_ICONERROR);
		return FALSE;
	}
	delete []buffer;
...
// km      2015-01-10                 add
//========================================================
void	_tagXMLNode::GetChildValueWchar( LPCTSTR name, WCHAR* outText)
{
	outText[0] = 0;
	LPXNode node = GetChild( name );
	if (node != NULL)
		swprintf(outText, L"%s", (LPCTSTR)node->value);
}

void	_tagXMLNode::GetChildValueChar( LPCTSTR name, LPSTR outTxt )
{
	outTxt[0] = 0;
	LPXNode node = GetChild( name );
	if (node != NULL)
	{
		sprintf(outTxt, "%S", (LPCTSTR)node->value);
	}
}

int	_tagXMLNode::GetChildValueInt( LPCTSTR name )
{
	LPXNode node = GetChild( name );
	if (node == NULL)
		return NULL;
